...
test('valid and invalid brick placement', () => {
  const bricks = [
    { id: "b1", type: "2x4" as const, position: [0, 0, 0] as [number,number,number], rotation: 0 }
  ];

  // Ground placement
  const groundRet = checkPlacementValid(bricks, { id: "g1", type: "2x4", position: [0.16, 0, 0], rotation: 0 }, MODULE_SIZE, BRICK_HEIGHT);
  assert.strictEqual(groundRet.valid, true);

  // Overlapping existing
  const overlapRet = checkPlacementValid(bricks, { id: "g2", type: "2x4", position: [0, 0, 0], rotation: 0 }, MODULE_SIZE, BRICK_HEIGHT);
  assert.strictEqual(overlapRet.valid, false);
  assert.strictEqual(overlapRet.reason, "overlap");

  // Supported above existing
  const supportedRet = checkPlacementValid(bricks, { id: "g3", type: "2x2", position: [0, BRICK_HEIGHT, 0], rotation: 0 }, MODULE_SIZE, BRICK_HEIGHT);
  assert.strictEqual(supportedRet.valid, true);

  // Floating placement
  const floatRet = checkPlacementValid(bricks, { id: "g4", type: "2x2", position: [0.32, BRICK_HEIGHT, 0], rotation: 0 }, MODULE_SIZE, BRICK_HEIGHT);
...
test('hasBrickAbove detection', () => {
  ...
  const b2: any = { id: "b2", type: "2x4", position: [0, BRICK_HEIGHT, 0], rotation: 0 };
...
test('clipboard preset helper', () => {
  const clipboardMock: any[] = [{ id: "c1", type: "2x2", position:[0,0,0], rotation:0, color:"#123" }];
...
test('structure support check', () => {
  const base = [
    { id: "base1", type: "2x4" as const, position: [0, 0, 0] as [number,number,number], rotation: 0 }
  ];
  const newBricks = [
    { id: "new1", type: "2x4" as const, position: [0, BRICK_HEIGHT, 0] as [number,number,number], rotation: 0 }
  ];
  ...
  const floatingBricks = [
    { id: "new2", type: "2x4" as const, position: [0.5, BRICK_HEIGHT, 0] as [number,number,number], rotation: 0 }
  ];
  const floatRes = checkStructureValid(base, floatingBricks, MODULE_SIZE, BRICK_HEIGHT);
